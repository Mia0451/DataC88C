...
def accuracy(typed, source):
    ...
    for i in range(len_min):
        if typed_words[i] == source_words[i]:
            num_correct_words += 1
    return num_correct_words / len_typed * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    current_pick = word_list[0]
    
    for source_word in word_list:
        dis = diff_function(typed_word, source_word, limit)
        if dis < current_distance:
            current_distance = dis
            current_pick = source_word
...
def furry_fixes(typed, source, limit):
    ...
    def diff(typed, source, idx, current_not_match):
        if current_not_match > limit:
            return current_not_match
        
        if idx >= len(typed) or idx >= len(source):
            return length_diff + current_not_match
        
        if typed[idx] == source[idx]:
            return diff(typed, source, idx + 1, current_not_match)
        else:
            return diff(typed, source, idx + 1, current_not_match + 1)
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    def recursive_edit(typed, idx_t, source, idx_s, current_edits):
        ...
        len_s = len(source)
        if idx_t >= len_t:
            return current_edits + len_s - idx_s
        elif idx_s >= len_s:
            return current_edits + len_t - idx_t
        
        min_edit = limit + 1
        if typed[idx_t] == source[idx_s]:
            edits = recursive_edit(typed, idx_t + 1, source, idx_s + 1, current_edits)
            if edits < min_edit:
                min_edit = edits
        
        add = recursive_edit(typed, idx_t, source, idx_s + 1, current_edits + 1)
        remove = recursive_edit(typed, idx_t + 1, source, idx_s, current_edits + 1)
        substitute = recursive_edit(typed, idx_t + 1, source, idx_s + 1, current_edits + 1)
        
        if add < min_edit:
            min_edit = add
        if remove < min_edit:
            min_edit = remove
        if substitute < min_edit:
            min_edit = substitute
        
        return min_edit
    
    return recursive_edit(typed, 0, source, 0, 0)
...
def time_per_word(words, timestamps_per_player):
    ...
    for tpp in timestamps_per_player:
        player_times = []
        for i in range(1, len(tpp)):
            player_times.append(tpp[i] - tpp[i - 1])
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        fastest_player = 0
        fastest_time = get_time(times, 0, word_index)
        for player_index in player_indices:
            player_time = get_time(times,player_index,word_index)
